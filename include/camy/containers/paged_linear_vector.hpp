...
namespace camy
{
    ...
    */
    ...
    inline PagedLinearVector<ElementType, kAlignment>::PagedLinearVector(rsize elements_per_page)
        : m_cur(nullptr), m_elements_per_page(elements_per_page)
    {
        _append_page();
    }
    ...
    template <typename ElementType, uint16 kAlignment>
    inline PagedLinearVector<ElementType, kAlignment>& PagedLinearVector<ElementType, kAlignment>::
    operator=(PagedLinearVector&& other)
    {
        m_cur = other.m_cur;
        m_elements_per_page = other.m_elements_per_page;
        other.m_cur = nullptr;
    ...
    inline void PagedLinearVector<ElementType, kAlignment>::_append_page()
    {
        if (m_cur == nullptr)
        {
            m_cur = API::tallocate<PageType>(CAMY_ALLOC1(kAlignment), m_elements_per_page);
        }
        else
        {
            PageType* old = m_cur;
