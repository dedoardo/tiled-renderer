...
namespace camy
{
	...
	*/
		...
		m_capacity(0)
	{
		...
		m_buffer = (ElementType*)API::allocate(CAMY_ALLOC(kElementSize * initial_capacity, kAlignment));
		m_cur = m_buffer;
	...
	inline ElementType& LinearVector<ElementType, kAlignment>::next()
	{
		...
		if (m_cur >= m_buffer + m_capacity)
		{
			...
			m_buffer = (ElementType*)API::allocate(CAMY_ALLOC(kElementSize * new_capacity, kAlignment));
