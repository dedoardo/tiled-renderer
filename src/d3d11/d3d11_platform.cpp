...
namespace camy
{
    ...
    HWND create_window(const StartupInfo& info, bool is_registered = false)
    {
        ...
        if (info.window.style == WindowDesc::Style::Popup) style = WS_POPUP;

        HWND ret_handle =
            CreateWindowA(class_name, info.window.name, style, CW_USEDEFAULT, CW_USEDEFAULT,
                          info.window.width, info.window.height, NULL, NULL, cur_instance, NULL);
        if (ret_handle == NULL)
        {
    ...
    namespace API
    {
        bool startup(const StartupInfo& info)
        {
            ...
            if (info.window.style != WindowDesc::Style::None)
            {
                ...
                if (g_runtime_info.whandle == nullptr) return false;
        ...
        void shutdown()
        {
            ...
            if (g_runtime_info.whandle != nullptr) destroy_window((HWND)g_runtime_info.whandle);
            ;
        ...
        RenderContext& render_context() { return *g_render_context; }

        RenderContext& rc() { return *g_render_context; }

        const RuntimeInfo& runtime_info() { return g_runtime_info; }

        const RuntimeInfo& info() { return g_runtime_info; }
        ...
        float4x4 to_shader_order(const float4x4& mat) { return transpose(mat); }
    }
}
#endif