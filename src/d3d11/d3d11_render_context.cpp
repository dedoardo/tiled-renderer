...
void set_debug_name(ID3D11DeviceChild* child,
                    const camy::char8* type,
                    const camy::char8* name,
                    const camy::char8* gen = nullptr,
                    int idx = 0)
{
    if (child == nullptr)
    {
...
namespace camy
{
    ...
    bool RenderContext::init(const StartupInfo& info_in, RuntimeInfo& info_out)
    {
        ...
        if (m_data.factory->EnumAdapters(info_in.preferred_device, &current_adapter) !=
            DXGI_ERROR_NOT_FOUND)
        {
            ...
            result = D3D11CreateDevice(current_adapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, flags,
                                       &feature_level, 1, D3D11_SDK_VERSION, &m_data.device_old,
                                       &fl, &m_data.immediate_context_old);
            ...
            if (SUCCEEDED(result))
            {
                ...
                if (SUCCEEDED(result))
                {
                    m_data.immediate_context_old->QueryInterface(__uuidof(ID3D11DeviceContext1),
    ...
    success:
        RECT window_rect;
        GetClientRect(
            (HWND)info_out.whandle,
            &window_rect); // GetWindowRect is not really correct and causes issued w/ alignment
        ...
        {
            UINT rquality;
            m_data.device->CheckMultisampleQualityLevels(sc_desc.BufferDesc.Format, info_in.msaa,
                                                         &rquality);
            if (rquality == 0)
            {
        ...
        Surface backbuffer;
        result = m_data.swap_chain->GetBuffer(0, __uuidof(ID3D11Texture2D),
                                              (void**)&backbuffer.native.texture2d);
        if (FAILED(result))
        {
            destroy();
        ...
        backbuffer.native.rtvs =
            API::tallocate_array<ID3D11RenderTargetView*>(CAMY_UALLOC(1), nullptr);

        result = m_data.device->CreateRenderTargetView(backbuffer.native.texture2d, nullptr,
                                                       &backbuffer.native.rtvs[0]);
        if (FAILED(result))
        {
            destroy();
                                            ...
                                            const char8* name)
    {
        ...
        if (desc.type == SurfaceDesc::Type::Surface2D)
        {
            if (desc.array_count > 1)
                CL_WARN("Creating Surface2D with greater than 1 surface count, use Surface2DArray "
                        "as type: ",
                        name);
            ndesc.ArraySize = 1;
        }
        ...
        {
            if (desc.array_count == 1)
                CL_WARN("Creating Surface2DArray with 1 surface count, you might want to use "
                        "Surface2D: ",
                        name);
        ...
        {
            if (desc.array_count > 1)
                CL_WARN("Creating SurfaceCube with greater than 1 surface count, use "
                        "SurfaceCubeArray as type: ",
                        name);
            ndesc.ArraySize = 6;
        }
        ...
        {
            if (desc.array_count == 1)
                CL_WARN("Creating SurfaceCubeArray with 1 surface count, you might want to use "
                        "SurfaceCube: ",
                        name);
        ...
        HRESULT result = m_data.device->CreateTexture2D(
            &ndesc, upload_data ? sub_resources.data() : nullptr, &texture);
        if (FAILED(result))
        {
        ...
        {
            uavs =
                API::tallocate_array<ID3D11UnorderedAccessView*>(CAMY_UALLOC(num_views), nullptr);
    ...
    RenderContext::create_buffer(const BufferDesc& desc, const void* data, const char8* name)
    {
        ...
        ndesc.BindFlags =
            D3D11_BIND_SHADER_RESOURCE | (desc.is_uav ? D3D11_BIND_UNORDERED_ACCESS : 0);
        ...
        HRESULT result =
            m_data.device->CreateBuffer(&ndesc, data == nullptr ? nullptr : &initial_data, &buffer);
        if (FAILED(result))
        {
                                                  ...
                                                  const char8* name)
    {
        ...
        HRESULT result =
            m_data.device->CreateBuffer(&ndesc, (data != nullptr) ? &vb_data : nullptr, &buffer);
        if (FAILED(result))
        {
            CL_ERR("ID3D11Device::CreateBuffer failed with error: ", result);
            goto error;
    ...
    D3D11_CULL_MODE camy_to_d3d11(RasterizerStateDesc::Cull cull_mode)
    {
        ...
        default:
            return D3D11_CULL_NONE; // Default d3d11 is back but None IMHO Makes the user notice
                                                     ...
                                                     const char8* name)
    {
        ...
        rs_desc.FrontCounterClockwise = rs_desc.DepthBias = desc.depth_bias;
    ...
    HResource RenderContext::create_input_signature(InputSignatureDesc& desc, const char8* name)
    {
        ...
        HRESULT result =
            m_data.device->CreateInputLayout(input_elements.data(), desc.num_elements,
                                             desc.bytecode.data, desc.bytecode.size, &input_layout);
        if (FAILED(result))
        {
    ...
    HResource RenderContext::create_sampler(const SamplerDesc& desc, const char8* name)
    {
        ...
        s_desc.Filter =
            camy_to_d3d11(desc.filter, desc.comparison != SamplerDesc::Comparison::Never);
        s_desc.AddressU = s_desc.AddressV = s_desc.AddressW = camy_to_d3d11(desc.address);
        s_desc.MinLOD = -FLT_MAX;
        s_desc.MaxLOD = FLT_MAX;
        s_desc.MipLODBias = 0.f;
        s_desc.MaxAnisotropy =
            desc.filter == SamplerDesc::Filter::Anisotropic ? D3D11_MAX_MAXANISOTROPY : 0;
        s_desc.ComparisonFunc = camy_to_d3d11(desc.comparison);
        s_desc.BorderColor[0] = s_desc.BorderColor[1] = s_desc.BorderColor[2] =
    ...
    HResource RenderContext::create_shader(const ShaderDesc& desc, const char8* name)
    {
        ...
        case ShaderDesc::Type::Vertex:
            result = m_data.device->CreateVertexShader(desc.bytecode.data, desc.bytecode.size,
                                                       nullptr, (ID3D11VertexShader**)&shader);
            break;
        case ShaderDesc::Type::Geometry:
            result = m_data.device->CreateGeometryShader(desc.bytecode.data, desc.bytecode.size,
                                                         nullptr, (ID3D11GeometryShader**)&shader);
            break;
        case ShaderDesc::Type::Pixel:
            result = m_data.device->CreatePixelShader(desc.bytecode.data, desc.bytecode.size,
                                                      nullptr, (ID3D11PixelShader**)&shader);
            break;
        case ShaderDesc::Type::Compute:
            result = m_data.device->CreateComputeShader(desc.bytecode.data, desc.bytecode.size,
                                                        nullptr, (ID3D11ComputeShader**)&shader);
            break;
        default:
            CAMY_ASSERT(false);
        }
