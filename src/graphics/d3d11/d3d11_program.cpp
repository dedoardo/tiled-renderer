...
namespace camy
{
	bool Program::impl_compile_stage(ShaderDesc::Type type, const CompileOpts& opts, const Blob& text, Blob& data_out)
	{
		...
		char8* target;
		if (type == ShaderDesc::Type::Vertex) target = "vs_5_0";
		else if (type == ShaderDesc::Type::Pixel) target = "ps_5_0";
		else if (type == ShaderDesc::Type::Geometry) target = "gs_5_0";
		else if (type == ShaderDesc::Type::Compute) target = "cs_5_0";
		else target = "error";
		...
		data_out.allocate_data((byte*)bytecode->GetBufferPointer(), (rsize)bytecode->GetBufferSize());
		return true;
	}
	...
	bool Program::reflect(ShaderDesc::Type shader_type, const Blob& data, const char8* name)
	{
		...
		for (UINT i = 0; i < bound_cbuffers; ++i)
		{
			...
			if (cbuffer_desc.Type != D3D_CT_CBUFFER)
				continue;
		...
		if (shader_type == ShaderDesc::Type::Vertex) // TODO: First vertex processing stage, could be geometry
		{
