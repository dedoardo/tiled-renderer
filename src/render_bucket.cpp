...
return m_items.next();
	...
	void _set_pipeline_state(CommandList& command_list, const PipelineState& pipeline_state)
	{
		...
		for (rsize i = 0; i < API::MAX_RENDER_TARGETS; ++i) render_targets[i] = pipeline_state.render_targets[i].resource;

		command_list.set_targets(render_targets, API::MAX_RENDER_TARGETS, pipeline_state.depth_buffer.resource);
...
#pragma pack(push, 1)
	struct PipelineStateCache
	{
		...
		ParameterBlock::CacheTag cache_tags[API::MAX_PARAMETER_BLOCKS]{ ParameterBlock::kInvalidCacheTag, ParameterBlock::kInvalidCacheTag , ParameterBlock::kInvalidCacheTag , ParameterBlock::kInvalidCacheTag , ParameterBlock::kInvalidCacheTag };
	};
#pragma pack(pop)
	...
	void RenderBucket::_compile()
	{
		...
		for (rsize i = 0; i < m_shared_parameters.count(); ++i)
		{
			...
			if (!param.var.valid())
			{
				CL_ERR("Trying to set invalid *shared* parameter, likely a call to Program::var() failed somewhere");
				continue;
			}
		...
		for (rsize i = 0; i < m_sorted_indices.count(); ++i)
		{
			...
			static_assert(API::MAX_VERTEX_BUFFERS == 2 && sizeof(HResource) == 2, "Code below relies on this assumption");
			...
			for (rsize b = 0; b < API::MAX_PARAMETER_BLOCKS; ++b)
			{
					...
					block.cache_tag != cache.cache_tags[b])
				{
					for (rsize p = 0; p < block.num_parameters; ++p)
					{
						...
						if (!param.var.valid())
						{
							CL_ERR("Trying to set invalid parameter, likely a call to Program::var() failed somewhere");
							continue;
						}
