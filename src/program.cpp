...
namespace camy
{
    ...
    bool Program::load_from_text(const ProgramDesc& desc, const char8* name)
    {
        ...
        if (desc.vertex_src.contains_data())
            compile_stage(ShaderDesc::Type::Vertex, desc.vertex_opts, desc.vertex_src,
                          bytecode_desc.vertex_src);
        if (desc.pixel_src.contains_data())
            compile_stage(ShaderDesc::Type::Pixel, desc.pixel_opts, desc.pixel_src,
                          bytecode_desc.pixel_src);
    ...
    bool Program::compile_stage(ShaderDesc::Type type,
    ...
    HResource Program::input_signature() { return m_input_signature; }

    HResource Program::shader(ShaderDesc::Type type) { return m_shaders[(rsize)type]; }

    HResource Program::vs() { return shader(ShaderDesc::Type::Vertex); }

    HResource Program::ps() { return shader(ShaderDesc::Type::Pixel); }
