...
namespace camy
{
		...
		for (rsize i = 0; i < command_list.m_data.init_requests.count(); ++i)
		{
			...
			if (req.type == InitResRequest::Type::Framebuffer)
			{
				...
				for (rsize s = 0; s < API::MAX_RENDER_TARGETS; ++s)
				{
					...
					glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + s, surface.native.texture, 0);
				}
				...
				{
					Surface& surface = get_surface(handle);
					glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, surface.native.texture, 0);
				}
	...
	HResource RenderContext::create_surface(const SurfaceDesc& desc, const SubSurface* subsurfaces, rsize num_subsurfaces, const char8* name)
	{
		...
		{
			CL_ERR("Invalid argument: SurfaceDesc::num_subsurfaces expected: ", expected_num_subsurfaces, " but got: ", num_subsurfaces);
			return HResource::make_invalid();
		}
