...
namespace camy
{
    ...
    bool Program::reflect(ShaderDesc::Type shader_type, const Blob& data, const char8* name)
    {
        ...
        ArrayAutoPtr<char8> ub_name =
            (char8*)API::allocate(CAMY_UALLOC(sizeof(char8) * max_ub_name_len));

        // Constant buffers
        for (GLint i = 0; i < active_ubs; ++i)
        {
            glGetProgramResourceName(program, GL_UNIFORM_BLOCK, i, max_ub_name_len, nullptr,
                                     ub_name);
            GLint idx = glGetUniformBlockIndex(program, ub_name);

            GLenum size_prop = GL_BUFFER_DATA_SIZE;
            GLint ub_size;
            glGetProgramResourceiv(program, GL_UNIFORM_BLOCK, i, 1, &size_prop, 1, nullptr,
                                   &ub_size);
        ...
        if (shader_type == ShaderDesc::Type::Vertex)
        {
            ...
            for (GLint i = 0; i < num_inputs; ++i)
            {
                ...
                GLint input_vals[2];
                glGetProgramResourceiv(program, GL_PROGRAM_INPUT, i, ARRAYSIZE(input_props),
                                       input_props, ARRAYSIZE(input_props), nullptr, input_vals);
