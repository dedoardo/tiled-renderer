...
struct name                                                                                        \
    {                                                                                              \
        static const ::camy::rsize kVal = idx;                                                     \
    };
...
namespace camy
{
    ...
    void CommandList::begin() { m_data.command_buffer.clear(); }
    ...
    void CommandList::clear_color(HResource handle, const float4& color)
    {
        ...
        Cmd::append<Cmd::ClearTexImage>(m_data, surface.native.texture, GLint(0), (GLenum)GL_RGBA,
                                        (GLenum)GL_FLOAT, (uint8)sizeof(float4), color);
    }
    ...
    void CommandList::clear_depth(HResource handle, float depth)
    {
        ...
        Cmd::append<Cmd::ClearTexImage>(m_data, surface.native.texture, GLint(0), (GLenum)GL_RGBA,
                                        (GLenum)GL_FLOAT, (uint8)sizeof(float), depth);
    }
    ...
    void CommandList::clear_stencil(HResource handle, uint32 val)
    {
        ...
        Cmd::append<Cmd::ClearTexImage>(m_data, surface.native.texture, GLint(0), (GLenum)GL_RGBA,
                                        (GLenum)GL_FLOAT, (uint8)sizeof(uint32), val);
    }

    void CommandList::set_vertex_shader(HResource handle) { m_data.cur_vertex_shader = handle; }
    ...
    void CommandList::set_pixel_shader(HResource handle) { m_data.cur_pixel_shader = handle; }

    CAMY_INLINE uint64 hash_targets(const HResource* render_targets,
                                    rsize num_render_targets,
                                    HResource depth_buffer)
    {
        static_assert(sizeof(HResource) == 2,
                      "Hashing based on this assumption, rewrite if it changes");
        uint64 hash = 0x0;
        rsize i = 0;
                                  ...
                                  uint8* views)
    {
        ...
        {
            CL_ERR("Invalid argument: num_render_targets must be between [0, ",
                   API::MAX_RENDER_TARGETS, "(API::MAX_RENDER_TARGETS)]");
            return;
        }
        ...
        if (m_data.fbo_map[hash] == nullptr)
        {
            ...
            static_assert(
                sizeof(HResource) == 2,
                "Code below relies on this assumption as HResource hasnt been defined yet");
    ...
    void CommandList::set_rasterizer_state(HResource handle)
    {
        ...
        Cmd::append<Cmd::PolygonMode>(
            m_data, (GLenum)GL_FRONT_AND_BACK,
        ...
        else
        {
            ...
            Cmd::append<Cmd::CullFace, GLenum>(
                m_data, rs.desc.cull == RasterizerStateDesc::Cull::Front ? GL_FRONT : GL_BACK);
        }

        // factor = slopeScaleDepthBias
        // units = depthBias
        Cmd::append<Cmd::PolygonOffsetClamp>(m_data, (float)rs.desc.slope_scaled_depth_bias,
    ...
    {
        Cmd::append<Cmd::Viewport>(m_data, (GLint)viewport.left, (GLint)viewport.top,
                                   (GLsizei)(viewport.right - viewport.left),
                                   (GLsizei)(viewport.bottom - viewport.top));

        Cmd::append<Cmd::DepthRange>(m_data, (GLfloat)viewport.near, (GLfloat)viewport.far);
    }
    ...
    void CommandList::set_index_buffer(HResource handle) { m_data.cur_index_buffer = handle; }

    void CommandList::set_cbuffer(ShaderVariable var, HResource handle)
    {
        ConstantBuffer& cb = API::rc().get_constant_buffer(handle);

        Cmd::append<Cmd::BindBufferBase>(m_data, (GLenum)GL_UNIFORM_BUFFER, (GLuint)var.slot(),
                                         cb.native.buffer);
    }
    ...
    void CommandList::set_cbuffer_off(ShaderVariable var, HResource handle, rsize offset)
    {
        ...
        Cmd::append<Cmd::BindBufferRange>(m_data, (GLenum)GL_UNIFORM_BUFFER, (GLuint)var.slot(),
                                          cb.native.buffer, (GLintptr)byte_offset,
                                          (GLsizeiptr)var.size());
    }
    ...
    CAMY_INLINE void validate_vao(CommandListData& data)
    {
        ...
        for (rsize i = 0; i < API::MAX_VERTEX_BUFFERS; ++i)
        {
            ...
            {
                VertexBuffer& vb =
                    API::rc().get_vertex_buffer((HResource)data.cur_vertex_buffers[i]);
                Cmd::append<Cmd::BindVertexBuffer>(data, (GLuint)i, vb.native.buffer, (GLintptr)0,
    ...
    void CommandList::draw(uint32 vertex_count, uint32 vertex_offset)
    {
        ...
        Cmd::append<Cmd::DrawArrays>(m_data, (GLenum)m_data.cur_primitive_topology,
                                     (GLint)vertex_offset, (GLsizei)vertex_count);
    }
    ...
    void CommandList::draw_indexed(uint32 index_count, uint32 index_offset, uint32 vertex_offset)
    {
        ...
        Cmd::append<Cmd::DrawElementsBaseVertex>(
            m_data, (GLenum)m_data.cur_primitive_topology, (GLsizei)index_count,
            (GLenum)(ib.desc.extended32 ? GL_UNSIGNED_INT : GL_UNSIGNED_SHORT),
            (char8*)0 + index_offset * index_size, (GLint)vertex_offset);
    }
                                     ...
                                     uint32 instance_offset)
    {
        ...
        Cmd::append<Cmd::DrawArraysInstancedBaseInstance>(
            m_data, (GLenum)m_data.cur_primitive_topology, (GLint)vertex_offset,
            (GLsizei)vertex_count, (GLsizei)instance_count, (GLuint)instance_offset);
    }
                                             ...
                                             uint32 instance_offset)
    {
        ...
        Cmd::append<Cmd::DrawElementsInstancedBaseVertex>(
            m_data, (GLenum)m_data.cur_primitive_topology, (GLsizei)index_count,
            (GLenum)(ib.desc.extended32 ? GL_UNSIGNED_INT : GL_UNSIGNED_SHORT),
            (char8*)0 + index_offset * index_size, (GLsizei)instance_count, (GLint)vertex_offset);
    }
